...
public class SkierService implements ISkierService {
    ...
    public Skier assignSkierToPiste(Long numSkier, Long numPiste) {
        Skier skier = iSikierRepository.findById(numSkier).orElseThrow(() -> new RuntimeException("Skier not found"));
        Piste piste = iPisteRepository.findById(numPiste).orElseThrow(() -> new RuntimeException("Piste not found"));
